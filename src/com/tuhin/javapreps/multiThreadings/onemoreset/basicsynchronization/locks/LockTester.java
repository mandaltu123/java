package com.tuhin.javapreps.multiThreadings.onemoreset.basicsynchronization.locks;
 ...
 * <p>
 * Synchronization is built around an internal entity known as the intrinsic lock or monitor lock. (The API specification
 * often refers to this entity simply as a "monitor.") Intrinsic locks play a role in both aspects of synchronization:
 * enforcing exclusive access to an object's state and establishing happens-before relationships that are essential to
 * visibility.
 * <p>
 ...
 * class. Thus access to class's static fields is controlled by a lock that's distinct from the lock for any instance of
 * the class.
 * <p>
 ...
 * synchronizing invocations of other objects' methods. (Invoking other objects' methods from synchronized code can
 * create problems that are described in the section on Liveness.) Without synchronized statements, there would have
 * to be a separate, unsynchronized method for the sole purpose of invoking nameList.add.
 * <p>
 * Synchronized statements are also useful for improving concurrency with fine-grained synchronization. Suppose, for
 * example, class MsLunch has two instance fields, c1 and c2, that are never used together. All updates of these fields
 * must be synchronized, but there's no reason to prevent an update of c1 from being interleaved with an update of c2 —
 * and doing so reduces concurrency by creating unnecessary blocking. Instead of using synchronized methods or otherwise
 * using the lock associated with this, we create two objects solely to provide locks.
 * <p>
